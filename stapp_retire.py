...
def main():
    ...
    if choice == 'グラフ表示':
        ...
        if 'df' in st.session_state:
            ...
            # セッションステートに退避していたデータフレームを復元
            df = copy.deepcopy(st.session_state.df)
            option= st.sidebar.selectbox('グラフのx軸',("退職", "年齢", "出勤頻度", "通勤距離", "最終学歴","社員番号","職場環境の満足度","労働意欲","査定ランク",
            "仕事への満足度","月給(ドル)","転職回数","昇給率","能力評価","人間関係の満足度","ストックオプション","社会人歴",
            "昨年の研修時間","ワークライフバランス","在職年数","現在の役職年数","昇進からの経過年数","現在の上司になってからの年数",
            "残業_有","性別_男性","配偶者_離婚","配偶者_結婚","配偶者_独身","部署_人事","部署_研究開発","部署_営業","専攻_経営",
            "専攻_科学","専攻_医学","専攻_工学","専攻_人事","専攻_その他","役職_経営層","役職_管理職","役職_主任","役職_一般"))
            
            # グラフの表示
            st_display_graph(df,option)
    ...
    if choice == '学習と検証':
        ...
        if 'df' in st.session_state:
            ...
            st.caption('')
            st.caption('決定木の可視化')
            st_display_dtree(clf,)#わからん

        else:
            st.subheader('訓練用データをアップロードしてください')
    if choice == 'About':
        image = Image.open('logo.ncc.png')
        st.image(image)
        st.markdown("Built by [NCC student]") 
        st.text("Version 1.0")
        st.markdown("For More Infomation check out (https://www.ncc-net.ac.jp)")
